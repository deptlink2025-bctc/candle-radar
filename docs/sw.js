/* Service worker: nhận Web Push + ép tải lại tài nguyên (chép từ tudoanh-radar/docs/sw.js).
...
self.addEventListener("push", (e) => {
  let p = {};
  try { p = e.data ? e.data.json() : {}; } catch (_) { p = { title: "Candle Radar", body: e.data && e.data.text() }; }
  const opts = {
    body: p.body || "",
  ...
  e.waitUntil(self.registration.showNotification(p.title || "Candle Radar", opts));
});
