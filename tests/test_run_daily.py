...
def test_job_ghi_file_va_push_theo_ma(site, sent, monkeypatch):
    ...
    assert cand["symbol"] == "HPG" and cand["tag"] == "cr-HPG" and "Nhấn chìm tăng" in cand["title"]
...
def test_xu_huong_di_rieng_khong_gop_vao_tong_hop(site, sent, monkeypatch):
    ...
    assert tr["title"] == "▼ THOÁT HPG · Supertrend lật đỏ" and tr["tag"] == "cr-st-HPG"
...
def test_trend_payload_mua_kem_dung_lo():
    ...
    assert p["kind"] == "trend" and p["tag"] == "cr-st-DCM"
...
def test_khong_co_danh_muc_thi_thoat_2(site, monkeypatch):
    monkeypatch.setattr(rd.watchlist, "load", lambda: ([], "none"))
    assert rd.run(no_push=True) == 2
